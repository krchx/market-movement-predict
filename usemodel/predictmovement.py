...
from data.preprocessing import load_and_clean_data, calculate_features, split_features

# --- Configuration ---
DEVICE = cfg.DEVICE
SEQ_LENGTH = cfg.SEQ_LENGTH
MODEL_PATH = 'models/hybrid_model_20250416_145635.pth' # Path to the trained model
SCALER_PATH = 'data/scaler.joblib' # Path to the scaler used during training
DATA_PATH = 'data.csv' # Path to the new day's data
...
FEATURE_COLS = ['open_return', 'high_return', 'low_return', 'close_return',
                'volume_change', 'volume_velocity', 'relative_position',
                'minute_sin', 'minute_cos', 'day_sin', 'day_cos'] # Example: Verify this list!
...
try:
    ...
    new_df_features = split_features(df_with_features)

    if new_df_features.empty:
        print("Error: No data left after loading and initial feature calculation.")
        exit()
...
# # Create a results DataFrame, aligning with original data using stored indices
# # Select the rows from the *feature* dataframe that correspond to the end of each sequence
# results_df = new_df_features.loc[result_indices].copy()
...
# display_cols = ['datetime', 'open', 'high', 'low', 'close', 'predicted_label', 'prob_down', 'prob_sideways', 'prob_up']
...
# results_df.to_csv(output_filename, index=True) # Keep index if it's the datetime
